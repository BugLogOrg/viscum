...
export function NotificationsClient({
  ...
  initialRows: RemoteNotify[];
  loggedIn: boolean;
}) {
  // SSR と hydration を揃えるため prefs 濾しは mount 後（localStorage）
  const [rows, setRows] = useState<LocalNotify[]>(
    loggedIn ? (initialRows as LocalNotify[]) : [],
  );
  ...
  useEffect(() => {
    ...
    setRows(applyPrefs(initialRows));
  }, [initialRows, loggedIn]);
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <button
          ...
          onClick={() => {
            ...
            void markAllRemoteNotifiesRead();
          }}
